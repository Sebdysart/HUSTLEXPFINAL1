...
struct EarningsScreen: View {
    ...
    private var earningsHero: some View {
        VStack(spacing: 8) {
            HXText("Total Earnings", style: .subheadline, color: .secondary)
...
struct EarningsPeriodRow: View {
    ...
    var body: some View {
        HStack {
            HXIcon(icon, size: .small, color: .secondary)
            HXText(period, style: .body, color: .secondary)
            Spacer()
        ...
        HStack {
            VStack(alignment: .leading, spacing: 2) {
                ...
                if let date = task.completedAt {
                    HXText(date.formatted(date: .abbreviated, time: .omitted), style: .caption, color: .secondary)
                }
            }
